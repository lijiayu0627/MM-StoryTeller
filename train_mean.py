...
from datasets.datasets import MeanDataset
from model.mean_clip import *
...
def test(model,test_set,prefix_length,tokenizer):
    ...
    for tokens, mask, prefix, video,cap in test_set:
        tokens, mask, prefix = tokens.to(device), mask.to(device), prefix.to(device,dtype=torch.float32)
...
def main():
    ...
    dataset = MeanDataset(args.data, prefix_length, normalize_prefix=args.normalize_prefix)
